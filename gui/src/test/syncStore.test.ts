  ...
  describe("sync_progress", () => {
    it("stores progress for the folder", () => {
      useSyncStore.getState()._handleEvent({
        type: "sync_progress",
        folder: "docs",
        progress: { done: 3, total: 10, current: "file.txt" },
      });
      expect(useSyncStore.getState().progress["docs"]).toEqual({ done: 3, total: 10, current: "file.txt" });
    });

    it("overwrites previous progress for the same folder", () => {
      useSyncStore.setState({ progress: { docs: { done: 1, total: 10, current: "a.txt" } } });
      useSyncStore.getState()._handleEvent({
        type: "sync_progress",
        folder: "docs",
        progress: { done: 5, total: 10, current: "b.txt" },
      });
  ...
  describe("sync_completed", () => {
    it("removes folder from syncing, clears its progress, records last_sync and counts", () => {
      useSyncStore.setState({
        ...
        progress: { docs: { done: 5, total: 5, current: "" } },
      });
      useSyncStore.getState()._handleEvent({
        type: "sync_completed",
        folder: "docs",
  ...
  describe("sync_failed", () => {
    it("removes folder from syncing and clears its progress", () => {
      useSyncStore.setState({
        ...
        progress: { docs: { done: 2, total: 10, current: "x.txt" } },
      });
